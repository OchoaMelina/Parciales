...
int loadFromText(char* path , LinkedList* pArrayList)
{
	int retorno=-1;
		int retornoLeido;
			...
			{
				retorno=0;
				if(ll_isEmpty(pArrayList)==1)
				{
					FILE *pFile=fopen(path,"r");
					retornoLeido=parser_FromText(pFile,pArrayList);
					if(retornoLeido!=0)
					{
...
int deleteLinkedList(LinkedList* pArrayListPassenger)
{
	int retorno=-1;
...
int ll_filter(LinkedList* lista, LinkedList* listaNew,int (*pFunc)(LinkedList* ,LinkedList*,int*))
{
	int retorno=-1;
	int opcion;
	if(lista!=NULL && listaNew!=NULL)
	{
		getNumero(&opcion,"Ingrese el tipo deseado 1.HEROE\t2.VILLANO\n","Error reingrese\n",2,1,2);
		(pFunc)(lista,listaNew,&opcion);
		retorno=0;
	}
	return retorno;
